...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!node) return pageMetadata({ title: "프로젝트를 찾을 수 없습니다", description: "요청한 프로젝트 글 페이지를 찾을 수 없습니다.", path: "/writing/projects" })
  return pageMetadata({
    title: `${node.name} 프로젝트 글`,
  ...
  return (
    <main className="min-h-screen bg-background paper-grain">
      ...
      <section className="mx-auto max-w-6xl px-5 py-14 md:px-8 md:py-20">
        ...
        <p className="mt-4 text-sm text-muted-foreground">{posts.length}편의 프로젝트 글</p>
      </section>
