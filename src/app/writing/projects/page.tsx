  ...
  return (
    <main className="min-h-screen bg-background paper-grain">
      ...
      <section className="mx-auto max-w-6xl px-5 pb-10 pt-16 md:px-8 md:pb-14 md:pt-24">
        <span className="label-tracked text-muted-foreground">P r o j e c t  P a g e s</span>
        <h1 className="mt-6 max-w-5xl font-serif-display text-5xl leading-[0.95] tracking-tight md:text-7xl">
          핵심 블로그에서 분리한
          <br />
          <span className="italic text-clay">프로젝트별 기록.</span>
        </h1>
        <p className="mt-8 max-w-3xl text-lg leading-relaxed text-muted-foreground md:text-xl">
          메인 글 목록은 PonsLink와 PonsWarp에 집중하고, 문서 자동화·도메인 AI·로컬 도구·운영 노트는 여기서 프로젝트 단위로 따로 읽도록 분리했습니다.
          흐름이 다른 글을 한 목록에 섞지 않기 위한 페이지입니다.
        </p>
        <p className="mt-5 text-sm text-muted-foreground">{projects.length}개 프로젝트 · {total}편의 글</p>
      </section>
      ...
      <section className="mx-auto grid max-w-6xl gap-4 px-5 pb-16 md:grid-cols-2 md:px-8 lg:grid-cols-3">
        {projects.map((project) => (
          ...
          >
            ...
            <p className="mt-4 line-clamp-4 text-sm leading-relaxed text-muted-foreground">{project.description || "프로젝트별로 분리한 글 모음."}</p>
