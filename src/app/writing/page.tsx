...
async function getArchiveData({
  ...
  for (const post of taxonomyPosts) {
    ...
    for (const item of splitTags(post.tags)) {
  ...
  if (tag) {
    posts = posts.filter((post) => splitTags(post.tags).includes(tag))
  }
...
export default async function WritingPage({ searchParams }: PageProps) {
  ...
  const view = archiveViews.some((option) => option.id === requestedView) ? (requestedView as ArchiveView) : "board"
  const viewLabel = archiveViews.find((option) => option.id === view)?.label ?? "Board"
  const { posts, categories, tags, totalPublished } = await getArchiveData({ category, tag, q })
        ...
        <div className="mx-auto grid max-w-6xl gap-6 px-5 py-6 md:px-8">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {categories.map((item) => (
              <Link
                key={item.name}
                href={archiveHref({ category: item.name, q, view })}
                className={`rounded-full border px-3.5 py-1.5 text-xs transition-colors ${
                  category === item.name
                    ? "border-foreground bg-foreground text-background"
                    : "border-border text-muted-foreground hover:border-foreground/40 hover:text-foreground"
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {tags.map((item) => (
                <Link
                  key={item.name}
                  href={archiveHref({ category, tag: item.name, q, view })}
        ...
        <div className="mb-5 flex flex-col gap-4 text-sm text-muted-foreground lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            <p>
              ...
              {category ? <span> · category: <span className="text-foreground">{category}</span></span> : null}
              {tag ? <span> · tag: <span className="text-foreground">#{tag}</span></span> : null}
              {q ? <span> · search: <span className="text-foreground">{q}</span></span> : null}
            </p>
            <div className="flex flex-wrap gap-2" aria-label="Archive viewer modes">
              {archiveViews.map((option) => (
                <Link
                  key={option.id}
                  href={archiveHref({ category, tag, q, view: option.id })}
