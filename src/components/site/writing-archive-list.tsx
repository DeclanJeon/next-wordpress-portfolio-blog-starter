      ...
      <div className="divide-y divide-border">
        {posts.map((post) => (
          <Link key={post.id} href={`/writing/${post.slug}`} className="group grid gap-3 px-5 py-4 transition-colors hover:bg-muted/40 md:grid-cols-[6rem_7rem_1fr_9rem_5rem] md:items-center">
            <ArchiveThumbnail post={post} className="h-16 w-24 md:h-12 md:w-20" />
            <span className="label-tracked-sm text-muted-foreground">{post.category}</span>
            <span>
    ...
    <div className="space-y-10">
      {groups.map((group) => (
        <section key={group.month} className="grid gap-4 md:grid-cols-[10rem_1fr]">
          <div>
            <p className="sticky top-20 font-serif-display text-2xl text-clay">{group.month}</p>
        ...
        {posts.map((post, index) => (
          <Link key={post.id} href={`/writing/${post.slug}`} className="group grid gap-2 py-3 text-sm transition-colors hover:text-clay md:grid-cols-[3rem_1fr_auto] md:items-center">
            ...
            <span>
              ...
              <span className="ml-2 hidden text-xs text-muted-foreground md:inline">{post.category}</span>
            </span>
  ...
  return (
    <div className="grid gap-px overflow-hidden rounded-2xl border border-border bg-border md:grid-cols-2">
      {posts.map((post) => {
        ...
        return (
          <Link key={post.id} href={`/writing/${post.slug}`} className="group relative flex min-h-72 flex-col justify-between bg-background p-7 text-left transition-colors hover:bg-muted/50 md:p-8">
            <div>
            ...
            <div className="absolute inset-x-0 bottom-0 h-[3px] w-0 transition-all duration-500 group-hover:w-full" style={{ backgroundColor: post.coverColor }} />
          </Link>
        )
      })}
    </div>
