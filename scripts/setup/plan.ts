...
function installPhases(ctx: SetupContext): Phase[] {
  ...
  phases.push({ name: "health-check", operations: [{ kind: "command", label: "Check systemd service", command: `systemctl is-active ${shellQuote(ctx.serviceName)}` }, { kind: "command", label: "Check local Next app", command: `for i in $(seq 1 30); do curl -fsS http://127.0.0.1:${ctx.appPort}/ >/dev/null && exit 0; sleep 1; done; curl -fsS http://127.0.0.1:${ctx.appPort}/ >/dev/null` }, { kind: "command", label: "Check public site", command: `for i in $(seq 1 30); do curl -fsS ${publicScheme(ctx)}://${ctx.domain}/ >/dev/null && exit 0; sleep 1; done; curl -fsS ${publicScheme(ctx)}://${ctx.domain}/ >/dev/null` }, { kind: "command", label: "Check WordPress REST", command: ctx.noWordpress ? "true" : `for i in $(seq 1 30); do curl -fsS ${publicScheme(ctx)}://${ctx.domain}/wp-json/ >/dev/null && exit 0; sleep 1; done; curl -fsS ${publicScheme(ctx)}://${ctx.domain}/wp-json/ >/dev/null` }] })
  return phases
}
