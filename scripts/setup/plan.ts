...
function preflightPhase(ctx: SetupContext): Phase {
  return {
    ...
    operations: [
      ...
      { kind: "command", label: "Check app port", command: `ss -ltn | grep -q ':${ctx.appPort} ' && echo 'port in use' || echo 'port available'` },
    ],
  }
}
...
function wordpressPhase(ctx: SetupContext): Phase {
  return {
    ...
    operations: [
      ...
      { kind: "command", label: "Download WordPress", command: `[ -f ${shellQuote(join(ctx.wordpressDir, "wp-load.php"))} ] || runuser -u www-data -- wp core download --path=${shellQuote(ctx.wordpressDir)} --locale=ko_KR` },
      { kind: "command", label: "Create wp-config.php", command: `[ -f ${shellQuote(join(ctx.wordpressDir, "wp-config.php"))} ] || runuser -u www-data -- wp config create --path=${shellQuote(ctx.wordpressDir)} --dbname=${shellQuote(ctx.dbName)} --dbuser=${shellQuote(ctx.dbUser)} --dbpass=${shellQuote(ctx.dbPassword)} --dbhost=localhost` },
      { kind: "command", label: "Install WordPress core", command: `runuser -u www-data -- wp core is-installed --path=${shellQuote(ctx.wordpressDir)} || runuser -u www-data -- wp core install --path=${shellQuote(ctx.wordpressDir)} --url=${shellQuote(`${publicScheme(ctx)}://${ctx.domain}`)} --title=${shellQuote(ctx.siteName)} --admin_user=${shellQuote(ctx.adminUser)} --admin_password=${shellQuote(ctx.adminPassword)} --admin_email=${shellQuote(ctx.email)}` },
      { kind: "command", label: "Set permalink", command: `runuser -u www-data -- wp option update permalink_structure '/%postname%/' --path=${shellQuote(ctx.wordpressDir)}` },
    ],
  }
}
...
function installPhases(ctx: SetupContext): Phase[] {
  ...
  phases.push(
    { name: "next-standalone", operations: [{ kind: "command", label: "Install JS dependencies", command: "bun install --frozen-lockfile" }, { kind: "command", label: "Build Next standalone", command: `set -a; . ${shellQuote(ctx.envFile)}; set +a; bun run build` }, { kind: "mkdir", label: "Create release dir", path: ctx.releaseDir }, { kind: "command", label: "Copy standalone server", command: `rsync -a --delete .next/standalone/ ${shellQuote(`${ctx.releaseDir}/`)}` }, { kind: "mkdir", label: "Create release .next dir", path: join(ctx.releaseDir, ".next") }, { kind: "command", label: "Copy static assets", command: `rsync -a --delete .next/static/ ${shellQuote(`${ctx.releaseDir}/.next/static/`)}` }, { kind: "command", label: "Copy public assets", command: `rsync -a --delete public/ ${shellQuote(`${ctx.releaseDir}/public/`)}` }, { kind: "command", label: "Initialize Prisma schema", command: `DATABASE_URL=${shellQuote(`file:${ctx.sqlitePath}`)} bunx prisma db push` }, { kind: "command", label: "Create local Next admin", command: `DATABASE_URL=${shellQuote(`file:${ctx.sqlitePath}`)} bun run scripts/create-local-admin.ts --username ${shellQuote(ctx.adminUser)} --password ${shellQuote(ctx.adminPassword)} --display-name ${shellQuote(ctx.siteName)} --role admin` }, { kind: "symlink", label: "Activate release", target: ctx.releaseDir, path: join(ctx.installDir, "current") }] },
